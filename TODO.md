# Master TODO - Rare Coin Project

> Central task tracking for all Rare Coin sub-projects

---

## 👥 Team & Assignments

| Assignee | Role | Tasks |
|----------|------|-------|
| **PaulySun** | Founder/Owner | Business decisions, contracts, final approvals |
| **Felix** | Director | Planning, coordination, frontend, docs |
| **Gemini 3.1** | Design Agent | Frontend design, UI/UX |
| **GLM5** | Backend Agent | Contracts, backend, DevOps |

---

## 📊 Project Status Overview

| Project | Status | Assignee | Priority |
|---------|--------|----------|----------|
| **rare-fyi (Marketing)** | 🟡 In Progress | Felix + Gemini | HIGH |
| **rare-coin (dApp)** | 🟡 In Progress | Felix | HIGH |
| **Contracts (Audit)** | 🟡 In Progress | GLM5 | CRITICAL |
| **Marketing Plan** | ✅ Complete | Felix | MEDIUM |
| **Supabase Setup** | 🟡 Configured | Felix | HIGH |

---

## 📅 4-Week Development Sprint

| Week | Focus | Status |
|------|-------|--------|
| **Week 1** | Core Contracts | ⏳ Pending |
| **Week 2** | Frontend dApp | ⏳ Pending |
| **Week 3** | Backend & Integration | ⏳ Pending |
| **Week 4** | Polish & Prep | ⏳ Pending |
| **After Week 4** | **ANNOUNCE** | ⏳ Decision Day |

---

## 🔥 Critical Path (Must Do First)

| # | Task | Project | Assignee | Status |
|---|------|---------|----------|--------|
| 1 | Write RareTokenV2.sol | contracts | Felix | ⏳ Week 1 |
| 2 | Write RareFountainV2.sol | contracts | Felix | ⏳ Week 1 |
| 3 | Write RareStakingV2.sol | contracts | Felix | ⏳ Week 1 |
| 4 | Write RareLottoV2.sol | contracts | Felix | ⏳ Week 1 |
| 5 | Write AirdropClaim.sol | contracts | Felix | ⏳ Week 1 |
| 6 | Deploy to Base testnet | contracts | Felix | ⏳ Week 1 |

---

## 📋 TODO by Project

### rare-fyi (Marketing Website)
See: [frontend/rare-fyi-todo.md](./frontend/rare-fyi-todo.md)

### rare-coin (dApp)
See: [frontend/rare-coin-todo.md](./frontend/rare-coin-todo.md)

### Contracts
See: [contracts/contracts-todo.md](./contracts/contracts-todo.md)

### Infrastructure
See: [infrastructure/infrastructure-todo.md](./infrastructure/infrastructure-todo.md)

### Marketing
See: [marketing/marketing-todo.md](./marketing/marketing-todo.md)

---

## 📅 This Week's Priorities

| Day | Focus | Tasks |
|-----|-------|-------|
| **Mon** | Website | Fix rare-fyi visual issues, push to GitHub |
| **Tue** | Contracts | Receive staking contract, begin audit |
| **Wed** | dApp | Begin rare-coin frontend overhaul |
| **Thu** | Infrastructure | Set up Supabase tables, test connections |
| **Fri** | Review | Review all progress, plan next week |

---

## 🚫 Blockers

| Blocker | Project | Blocked By | Resolution |
|---------|---------|------------|------------|
| Visual issues on rare-fyi | rare-fyi | Unknown | Need to audit live site |
| Missing staking contract | contracts | PaulySun | Waiting for file |
| GLM5 rate limit | Backend | Plan limits | Work around or wait |

---

## ✅ Recently Completed

| Task | Project | Completed | By |
|------|---------|-----------|-----|
| Pink/purple/cyan redesign | rare-fyi | 2026-02-24 | Felix |
| Blog post #1 (Why Failed) | marketing | 2026-02-24 | Felix |
| Blog post #2 (Bot Problem) | marketing | 2026-02-24 | Felix |
| Master TODO system | planning | 2026-02-24 | Felix |
| Lottery contract audit | contracts | 2026-02-24 | Felix |
| Marketing plan (90-day) | marketing | 2026-02-24 | Felix |
| Supabase schema | infrastructure | 2026-02-24 | Felix |
| rare-plan repo created | planning | 2026-02-24 | Felix |
| Token contract audit | contracts | 2026-02-24 | Felix |
| Bot-proofing strategy | contracts | 2026-02-24 | Felix |

---

## 📝 Notes

- Update this file daily
- Mark tasks as 🔴 Blocked / 🟡 In Progress / ✅ Done
- Add new tasks to appropriate project TODO
- Move completed tasks to "Recently Completed"

---

*Last Updated: 2026-02-24*
